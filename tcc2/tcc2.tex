...
\local{São Paulo}
\data{2026}
\orientador{Prof. Douglas José Baquião Ribeiro}
...
técnico, erros ortográficos e expressões coloquiais \cite{zemp2021}. 
Isso sobrecarrega os analistas, tornando a triagem manual lenta, 
inconsistente e propensa a erros.

Estudos recentes demonstram que sistemas automatizados com 
\textit{Transformers} alcançam até 92\% de acurácia em classificação 
multi-categoria \cite{zemp2021}, mas sua natureza de ``caixa-preta'', 
modelos que produzem predições sem revelar os critérios utilizados 
para tomá-las, gera resistência entre equipes operacionais que 
precisam confiar nas decisões algorítmicas para roteamento 
\cite{razma2026}.

A raiz matemática dessa instransparência está no mecanismo central 
desses modelos. A cada camada de processamento, o modelo realiza 
operações do tipo:
...
onde $Q$, $K$ e $V$ são matrizes de consulta, chave e valor 
derivadas da entrada \cite{vaswani2017}. Essas transformações 
se repetem em centenas de milhões de parâmetros distribuídos 
em múltiplas camadas, tornando impossível rastrear por que uma 
palavra específica influenciou determinada classificação. Embora 
o modelo produza resultados precisos, o caminho percorrido até 
a decisão permanece não auditável, ou seja, não é possível rastrear 
nem reproduzir a explicação de uma predição individual, propriedade 
definida em detalhe na Seção~\ref{subsec:auditabilidade}. Isso 
justifica a necessidade de técnicas de explicabilidade como SHAP e 
LIME para atribuir, de forma matematicamente justificável, a 
importância de cada termo na predição \cite{lundberg2017, ribeiro2016}.
...
MLP (\textit{Multilayer Perceptron}), que são redes neurais simples 
sem capacidade de entender o contexto das palavras, apresentam 
limitações significativas ao lidar com \textit{tickets} reais, 
pois não conseguem capturar a variabilidade semântica nem o 
vocabulário técnico específico do domínio de TI \cite{yokota2025}.
...
Investigar se a inclusão de uma camada de explicabilidade e grafos de co-ocorrência, a um \textit{pipeline} de classificação baseado em \textit{transformers}, aumenta a auditabilidade do processo decisório, em relação a abordagens sem explicabilidade, utilizando chamados de suporte de TI em português brasileiro.

\subsection{Objetivos secundários}

\begin{enumerate}

    \item Analisar o ganho de desempenho do modelo BERTimbau com 
    \textit{fine-tuning} em relação ao \textit{baseline} SVM com 
    TF-IDF, utilizando a métrica F1-macro.

    \item Comparar a explicabilidade gerada por SHAP e LIME em 
    termos de consistência, estabilidade e relevância semântica 
    das explicações produzidas para chamados de suporte técnico 
    em português brasileiro.

    \item Analisar se a visualização em grafos de co-ocorrência 
    ponderados por valores SHAP e LIME fornece auditabilidade 
    adicional ao processo de classificação em relação à exibição 
    isolada de \textit{scores} de importância.

    \item Validar a consistência semântica das explicações 
    geradas pelo \textit{pipeline} no contexto de chamados de 
    suporte técnico em português brasileiro, verificando se os 
    termos destacados correspondem ao vocabulário técnico 
    esperado para cada categoria classificada.
...
que permite que cada \textit{token}, ou seja, cada unidade mínima de texto em que 
o modelo divide a entrada, ``observe'' todos os outros da sequência, 
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Confiança, explicabilidade e adoção em produção}
...
Para modelos baseados em \textit{transformers}, o \textit{KernelSHAP} é a variante mais utilizada. O resultado é um vetor de importância por \textit{token} que pode ser visualizado como mapa de calor sobre o texto de entrada, \textit{tokens} com valores SHAP positivos contribuem para a classe predita, e \textit{tokens} com valores negativos contribuem contra ela. O presente trabalho pretende utilizar esses valores SHAP por \textit{token} como pesos dos nós no grafo de co-ocorrência, em conjunto com os valores LIME, permitindo comparação visual entre os dois métodos de explicabilidade.
...
A decisão de implementar e comparar SHAP e LIME neste trabalho, em vez de escolher apenas um dos métodos, é fundamentada em duas razões. Primeiro, a comparação entre os dois métodos produz evidência empírica sobre a qualidade e consistência das explicações geradas para chamados de TI em PT-BR, contribuição metodológica relevante para a literatura sobre XAI em domínios especializados. Segundo, a construção do grafo de co-ocorrência proposto utiliza os 
valores SHAP e LIME como pesos dos nós, permitindo avaliar 
se a escolha do método de XAI afeta a estrutura e a 
interpretabilidade do grafo resultante.
...
Um grafo de co-ocorrência de texto é uma estrutura 
$G = (V, E, W, f_{\text{shap}}, f_{\text{lime}})$, onde:

\begin{itemize}
    \item $V$ representa o conjunto de nós, correspondendo aos 
    \textit{tokens} únicos do texto;
    
    \item $E$ representa o conjunto de arestas, conectando pares 
    de \textit{tokens} que co-ocorrem dentro de uma janela 
    deslizante de tamanho $w$;
    
    \item $W$ representa os pesos das arestas, determinados pela 
    frequência de co-ocorrência entre os \textit{tokens};
    
    \item $f_{\text{shap}}$ representa os pesos dos nós calculados 
    pelo método SHAP, indicando a contribuição de cada 
    \textit{token} para a predição do modelo \cite{lundberg2017};
    
    \item $f_{\text{lime}}$ representa os pesos dos nós calculados 
    pelo método LIME, permitindo comparação visual entre os dois 
    métodos de explicabilidade \cite{ribeiro2016}.
\end{itemize}

A janela deslizante percorre o texto posição por posição; em cada 
posição, todos os pares de \textit{tokens} dentro da janela 
recebem uma aresta ou têm o peso de sua aresta existente 
incrementado \cite{yao2019}. Essa representação captura informação 
estrutural sobre o texto que as representações vetoriais não 
preservam: dois \textit{tokens} que frequentemente aparecem juntos 
no mesmo contexto estarão conectados por uma aresta de alto peso 
no grafo, independentemente de sua posição absoluta na sequência. 
Para chamados de TI, isso significa que termos técnicos que tendem 
a co-ocorrer em chamados de uma mesma categoria, como ``VPN'' e 
``acesso'' em chamados de conectividade, estarão estruturalmente 
próximos no grafo, com pesos de nós proporcionais à sua 
importância explicativa calculada por SHAP e LIME.
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Ponderação de arestas — frequência, PMI}
...
No presente trabalho, os pesos das arestas serão determinados pela frequência de co-ocorrência normalizada, enquanto os pesos dos \textit{nós}, que constituem o elemento central da proposta, serão determinados pelos valores SHAP e LIME calculados para cada \textit{token} pelo modelo BERTimbau, permitindo comparar visualmente a importância atribuída por cada método. Essa distinção entre ponderação de arestas (estrutura relacional) e ponderação de nós (relevância explicativa) é a inovação central da camada de visualização proposta.
...
RoBERTa, aplicará os valores SHAP e LIME como pesos dos nós em vez dos 
...
opaca do modelo em uma representação auditável, o que responde à 
limitação apontada por \citeonline{zemp2021} e \citeonline{razma2026}: 
sem acesso às razões da decisão, analistas de suporte não conseguem 
verificar o comportamento do modelo. O presente trabalho avalia a 
propriedade técnica de auditabilidade da representação proposta, sem 
medir a adoção efetiva do sistema por usuários. A solução proposta 
adota essa abordagem, cuja implementação é detalhada no 
...
O trabalho de \citeonline{zemp2021} demonstra que sistemas de classificação de chamados de TI com alta acurácia são tecnicamente viáveis e já foram implantados em produção. No entanto, a ausência de mecanismos de explicabilidade limita a adoção e a auditabilidade desses sistemas. Os agentes de suporte recebem predições sem justificativa, o que dificulta a identificação de erros sistemáticos e a construção de confiança no sistema. Nenhum dos trabalhos de classificação de chamados de TI identificados no \textit{screening} incorpora uma camada de visualização gráfica para explanações XAI.
...
\citeonline{wahba2023} questiona o uso de BERT para classificação de texto em domínios especializados, 
argumentando que o custo computacional adicional não se justifica diante da pequena vantagem sobre 
o SVM em textos técnicos com baixa polissemia. Esse argumento permanece válido enquanto o único critério 
de avaliação for a acurácia. O presente trabalho responde a esse argumento demonstrando que o custo 
do BERTimbau se justifica quando a camada de XAI e grafo de 
co-ocorrência é adicionada, transformando a saída opaca do modelo em 
uma representação auditável e verificável, critério que modelos 
lineares não conseguem satisfazer com a mesma qualidade de explanação.

% ---------------------------------------------------------------
\subsection{Direcionamento para a proposta}
\label{subsec:sintese_direcao}

Diante das lacunas identificadas: ausência de XAI em sistemas de classificação de chamados em produção, carência de soluções robustas para PT-BR em \textit{helpdesk} de TI, e a questão do custo-benefício do BERT sem explicabilidade, o presente trabalho investiga um \textit{pipeline} que combina BERTimbau, SHAP, LIME e grafo de co-ocorrência ponderado para classificação de chamados em PT-BR com explicabilidade visual. A descrição detalhada da arquitetura, do \textit{dataset}, dos experimentos e da interface que será desenvolvida é apresentada no Capítulo~\ref{cap:desenvolvimento}.
...
Um grafo de co-ocorrência é uma estrutura $G = (V, E, W, f_{\text{shap}}, f_{\text{lime}})$ na qual os nós $V$ representam os termos únicos de um texto e as arestas $E$ conectam pares de termos que aparecem juntos dentro de uma janela deslizante de tamanho fixo \cite{yao2019}. Neste trabalho, essa estrutura será utilizada como camada de visualização das explicações XAI: os pesos dos nós serão determinados pelos valores SHAP e LIME de cada \textit{token}, e os pesos das arestas pela frequência de co-ocorrência, produzindo uma representação visual que combina estrutura relacional do texto com relevância explicativa do modelo.
 
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{Auditabilidade}
\label{subsec:auditabilidade}

A literatura de XAI não apresenta uma definição única e consolidada de
auditabilidade. Por essa razão, adota-se aqui uma definição operacional,
construída a partir de propriedades já estabelecidas na literatura: um
sistema de classificação é auditável quando permite rastrear, verificar
e reproduzir a explicação de cada decisão do modelo. Trata-se, portanto,
de uma propriedade técnica do sistema, e não da confiança percebida por
quem o utiliza. Essa distinção é relevante porque os estudos de
\citeonline{zemp2021} e \citeonline{razma2026}, ao relatarem a reação de
analistas de suporte a sistemas de classificação em produção, tratam da
confiança percebida, e não da propriedade técnica definida aqui.

A definição se desdobra em três dimensões:

\begin{itemize}
    \item \textbf{Rastreabilidade}: a capacidade de atribuir a decisão
    do modelo a termos específicos da entrada, obtida por meio dos
    valores SHAP \cite{lundberg2017} e LIME \cite{ribeiro2016};

    \item \textbf{Estabilidade}: a reprodutibilidade da explicação
    gerada para uma mesma instância ao longo de execuções repetidas,
    questão especialmente relevante para o LIME, que depende de
    amostragem aleatória de perturbações \cite{salih2023};

    \item \textbf{Fidelidade}: o grau em que a explicação reflete o
    processo de decisão efetivamente utilizado pelo modelo
    \cite{jacovi2020}, verificável pela variação da confiança preditiva
    ao remover da entrada os termos apontados como mais relevantes.
\end{itemize}

A validação da percepção de confiança por parte de gestores ou analistas
de TI não é objeto deste trabalho, sendo reconhecida como extensão
natural da pesquisa.
...
A utilização de modelagem em grafos como camada de explicabilidade visual para o modelo BERT aumenta a auditabilidade do processo de classificação automatizada, permitindo identificar quais termos do chamado contribuíram para a categoria atribuída pelo modelo.
...
versões modificadas do texto. Os valores SHAP e LIME serão 
utilizados como pesos dos nós em um grafo de co-ocorrência 
$G = (V, E, W, f_{\text{shap}}, f_{\text{lime}})$, produzindo 
uma visualização gráfica que combina a estrutura relacional do 
texto com a relevância explicativa de cada termo segundo ambos 
os métodos.

O sistema será desenvolvido para atender principalmente gestores e 
analistas de TI, que poderão utilizá-lo durante o processo de triagem 
para acompanhar de forma transparente e auditável o resultado da 
classificação automática. A interface funciona como prova de conceito 
do \textit{pipeline}, não sendo avaliada junto a esse público neste 
trabalho.
...
A comparação entre os cenários A e B avalia o ganho de desempenho do BERTimbau sobre o \textit{baseline} clássico — respondendo ao argumento de \citeonline{wahba2023} sobre o custo-benefício dos \textit{transformers} em domínios especializados. A comparação entre os cenários C e D constitui o principal teste experimental da hipótese de pesquisa (Seção~\ref{sec:hipotese_pesquisa}), avaliando se a camada de grafo de co-ocorrência oferece auditabilidade superior à exibição isolada de scores de importância, nos termos definidos na Seção~\ref{subsec:auditabilidade}.
...
\begin{itemize}
    ...
    cada nó será proporcional ao valor SHAP e LIME do termo, indicando sua 
...
\begin{enumerate}
    ...
    \item \textbf{Construção do grafo de co-ocorrência} — os \textit{tokens} do chamado são representados como nós de um grafo $G = (V, E, W, f_{\text{shap}}, f_{\text{lime}})$. As arestas conectam pares de \textit{tokens} que co-ocorrem dentro de uma janela deslizante de tamanho fixo, com pesos proporcionais à frequência de co-ocorrência. Os pesos dos \textbf{nós} são determinados pelos valores SHAP e LIME calculados na etapa anterior — \textit{tokens} com maior importância SHAP geram nós visualmente maiores na interface. O grafo é serializado em JSON com a estrutura \texttt{\{nodes, edges\}} e enviado ao \textit{frontend}.
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
% VERSÃO FINAL: usar somente com os dois prints (Aché e Bayer)
% já salvos em imagens/. Antes disso, manter a versão que
% menciona apenas a Aché.
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Origem e autorização de uso dos dados}
\label{subsubsec:autorizacao_dados}

Os dados utilizados neste trabalho são chamados reais de suporte
técnico cedidos por duas empresas do setor farmacêutico: Aché
Laboratórios Farmacêuticos e Bayer S.A. Os autores solicitaram o
acesso por e-mail, explicando o objetivo acadêmico do trabalho, quais
informações seriam necessárias e como os registros seriam tratados.

As autorizações de uso foram concedidas pelas duas empresas por e-mail,
com a condição de que os chamados fossem anonimizados antes de qualquer
etapa de processamento. Essas mensagens são o registro da autorização
concedida e delimitam a finalidade acadêmica do uso, e estão
reproduzidas nos Anexos~\ref{an:autorizacao_ache}
e~\ref{an:autorizacao_bayer}.

A anonimização, além de atender a essa condição, alinha o trabalho ao
tratamento que a Lei Geral de Proteção de Dados Pessoais (LGPD, Lei
n.º 13.709/2018) dá aos dados anonimizados, que não são considerados
dados pessoais para os fins da lei, salvo quando a anonimização puder
ser revertida com esforços razoáveis \cite{brasil2018}. A etapa
descrita na Seção~\ref{subsubsec:dataset_finetuning} deixa de ser,
portanto, apenas uma escolha metodológica.



\subsubsection{\textit{Dataset} e \textit{fine-tuning}}
\label{subsubsec:dataset_finetuning}
O modelo BERTimbau será ajustado utilizando um conjunto de chamados de dados reais obtidos junto à Aché e Bayer Brasil, mediante autorização concedida por e-mail e anonimização dos registros.
...
A segunda perspectiva, \textbf{qualitativa}, avalia a utilidade das explicações geradas. 
Serão analisadas: a concordância entre as explicações SHAP e LIME para os mesmos chamados, e 
a coerência semântica dos \textit{tokens} destacados no grafo em relação à categoria predita, 
verificando se os termos com maior importância SHAP e LIME correspondem ao 
vocabulário técnico esperado para cada categoria. Complementarmente, 
a fidelidade é aferida pela biblioteca \textit{ferret} \cite{attanasio2023}, a estabilidade
pela repetição de SHAP e LIME sobre a mesma instância, e a rastreabilidade pela modularidade do grafo de co-ocorrência.
...
\chapter{Autorização de uso de dados --- Aché Laboratórios Farmacêuticos}
\label{an:autorizacao_ache}

Mensagem de e-mail em que a Aché Laboratórios Farmacêuticos autoriza o
uso de chamados de suporte técnico para fins acadêmicos neste trabalho,
mediante anonimização dos registros. Nomes, e-mails, ramais e
assinaturas dos remetentes foram removidos da imagem, mantendo-se
apenas o conteúdo da autorização e a identificação da empresa.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{imagens/autorizacao_ache.png}
    \caption{Autorização de uso de dados concedida pela Aché Laboratórios Farmacêuticos}
    \label{fig:autorizacao_ache}
    \legend{Fonte: Correspondência recebida pelos autores (2026)}
\end{figure}

\chapter{Autorização de uso de dados --- Bayer S.A.}
\label{an:autorizacao_bayer}

Mensagem de e-mail em que a Bayer S.A. autoriza o uso de chamados de
suporte técnico para fins acadêmicos neste trabalho, mediante
anonimização dos registros. Nomes, e-mails, ramais e assinaturas dos
remetentes foram removidos da imagem, mantendo-se apenas o conteúdo da
autorização e a identificação da empresa.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{imagens/autorizacao_bayer.png}
    \caption{Autorização de uso de dados concedida pela Bayer S.A.}
    \label{fig:autorizacao_bayer}
    \legend{Fonte: Correspondência recebida pelos autores (2026)}
\end{figure}
